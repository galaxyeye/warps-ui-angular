...
function useSetting(){
  ...
  var _keyboardBg = jQuery('#keyboardBg');
...
function refreshIco(){
  for (var i = 48; i < 91; i++) {
    var u = getCookie(""+i);
    if (u){
      jQuery("#"+i).parent().append("<img class='fav' src='"+ getico(u) +"' />");
    }
  }
}
/*-----------REFRESHICO END-----------*/
...
jQuery(document).ready(function(){
  ...
  /*this is for extensions
   function attention(){
   jQuery('.attention').addClass('attentionBlink');
   setTimeout("jQuery('.attention').removeClass('attentionBlink')",1600);
   }
   attention();
  ...
  jQuery(document).keydown(function(event){
    ...
    jQuery("#"+keyCode).addClass('keyDownLi');
    jQuery("#"+keyCode).parent().addClass('keyDownSpan');
  });
  ...
  jQuery(document).keyup(function(event){
    ...
    if (keyCode == 9 || event.ctrlKey || _search.is(":focus") || jQuery("#setting").is(":visible")){
      window.event.returnValue = false;
      return false;
    }else{
  ...
  jQuery("#keyboard ul span").mouseenter(function(){
    jQuery(this).append("<div class='keySetting'><img id='deleteImg' class='deleteImg' src='/assets/delete.png'/><img id='settingImg' class='settingImg' src='/assets/setting.png'/></div><div class='keySettingFix'></div>");//keySettingFix is for Firefox
    jQuery("#settingImg").click(function(){
    ...
    jQuery(".keySetting").fadeTo(500, 0.9);
  });

  jQuery("#keyboard ul span").mouseleave(function(){
    jQuery(this).children('[class^=keySetting]').remove();
  });

  jQuery("#keyboard ul span li").click(function(){
  ...
  _search.bind('input propertychange', function() {
    if (_search.val() == 'setting' || _search.val() == '设置'){
      ...
      jQuery("#setting img").click(function(){
        ...
        setTimeout("jQuery('#settingDetail').fadeIn(500)",300);
        //read setting
        newWindow && (jQuery('input[name=newWindow]')[0].checked = true);
        var now_searchEngine = getCookie('searchEngine');
        now_searchEngine == 'Google' && (jQuery('input[name=searchEngine]')[0].checked = true);
        now_searchEngine == 'Bing' && (jQuery('input[name=searchEngine]')[1].checked = true);
        now_searchEngine == 'Baidu' && (jQuery('input[name=searchEngine]')[2].checked = true);
        now_searchEngine == 'Other' && (jQuery('input[name=searchEngineURL]')[0].value = getCookie('searchEngineURL')) && (jQuery('input[name=searchEnginePOST]')[0].value = getCookie('searchEnginePOST'));
        jQuery('input[name=backgroundURL]')[0].value = getCookie('backgroundURL');
        //////////////
        jQuery('input[name=searchEngine]').click(function(){
          jQuery('input[name=searchEngineURL]')[0].value = '';
          jQuery('input[name=searchEnginePOST]')[0].value = '';
        });
        var _searchEngine = jQuery('input[name=searchEngine]');
        jQuery('input[name^=searchEngine]').bind('input propertychange', function() {
          for (var i=0;i<3;i++){_searchEngine[i].checked = false;}
        });

        //function for cancel and ok
        function settingFateOut(){
          jQuery('#settingDetail').fadeOut(500);
          setTimeout(function(){
            ...
            _setting.animate({
              ...
              marginLeft:'-200px',
              opacity:'0'
            },500,function(){jQuery('#setting').html("<p>&emsp;&emsp;&emsp;如果您无意修改设置请将此忽视：</p><img src='../img/setting.png'/>")
        ...
        jQuery("#OK").click(function(){
          setCookie('newWindow','');
          jQuery('input[name=newWindow]')[0].checked && setCookie('newWindow','t');
          for (var i=0;i<3;i++){_searchEngine[i].checked && setCookie('searchEngine',_searchEngine[i].value);}
          var _searchEngineURL = jQuery('input[name=searchEngineURL]')[0].value;
          var _searchEnginePOST = jQuery('input[name=searchEnginePOST]')[0].value;
          ...
          setCookie('backgroundURL',buildURL(jQuery('input[name=backgroundURL]')[0].value));
          useSetting();
          settingFateOut();
        });
